...
def open_encryption_decryption_app():
    try:
        ...
        if os.path.exists(script_path):
            ...
            exit_application()  # Safely close the app after launching
        else:
...
def exit_application():
    ...
    if gif_update_id is not None:
        root.after_cancel(gif_update_id)  # Cancel any pending after calls
    if root.winfo_exists():
        root.destroy()

# Function to update the GIF frames
def update_gif(label, gif_frames, frame_index):
    global gif_update_id
    if not root.winfo_exists():  # Check if the root window still exists
        return
    frame = gif_frames[frame_index]
    frame_index = (frame_index + 1) % len(gif_frames)
    label.configure(image=frame)
    gif_update_id = root.after(50, update_gif, label, gif_frames, frame_index)  # Reduced delay to increase speed
